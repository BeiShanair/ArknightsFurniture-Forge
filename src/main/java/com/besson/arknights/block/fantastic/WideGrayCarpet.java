...
public class WideGrayCarpet extends FurnitureHorizontalFacingBlock {
    ...
    public WideGrayCarpet(Properties settings) {
        super(settings);
    }
    ...
    public void setPlacedBy(Level world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        ...
        world.setBlock(right, ModBlocks.WIDE_GRAY_CARPET_RIGHT.get().defaultBlockState().setValue(FACING, dir),3);
    }
    ...
    private BlockPos findMainPos(BlockState state, BlockPos pos){
        if (state.is(ModBlocks.WIDE_GRAY_CARPET_LEFT.get())){
            return pos;
        }
        Direction dir = state.getValue(FACING);
        if (state.is(ModBlocks.WIDE_GRAY_CARPET_RIGHT.get())){
    ...
    private boolean isBookcase(BlockState state){
        return state.is(this) ||
                state.is(ModBlocks.WIDE_GRAY_CARPET_LEFT.get()) ||
                state.is(ModBlocks.WIDE_GRAY_CARPET_RIGHT.get());
    }
}
