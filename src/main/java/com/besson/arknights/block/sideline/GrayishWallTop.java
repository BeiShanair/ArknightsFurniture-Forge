...
public class GrayishWallTop extends FurnitureHorizontalFacingBlock {
    ...
    public GrayishWallTop(Properties settings) {
        super(settings);
    }
    ...
    public void setPlacedBy(Level world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        ...
        world.setBlock(bbr, ModBlocks.GRAYISH_WALL_TBR.get().defaultBlockState().setValue(FACING, dir), 3);
        world.setBlock(btl, ModBlocks.GRAYISH_WALL_TTL.get().defaultBlockState().setValue(FACING, dir), 3);
        world.setBlock(btr, ModBlocks.GRAYISH_WALL_TTR.get().defaultBlockState().setValue(FACING, dir), 3);
    }
    ...
    private BlockPos findMainBlockPosition(BlockState state, BlockPos pos) {
        ...
        if (state.is(ModBlocks.GRAYISH_WALL_TBL.get())) {
            return pos;
        }
        Direction dir = state.getValue(FACING);
        if (state.is(ModBlocks.GRAYISH_WALL_TBR.get())){
            return pos.relative(dir.getClockWise());
        } else if (state.is(ModBlocks.GRAYISH_WALL_TTL.get())){
            return pos.below();
        } else if (state.is(ModBlocks.GRAYISH_WALL_TTR.get())) {
    ...
    private boolean isWallBottom(BlockState state){
        return state.is(this) ||
                state.is(ModBlocks.GRAYISH_WALL_TBL.get()) ||
                state.is(ModBlocks.GRAYISH_WALL_TBR.get()) ||
                state.is(ModBlocks.GRAYISH_WALL_TTL.get()) ||
                state.is(ModBlocks.GRAYISH_WALL_TTR.get());
    }
}
